...
> Maximization of the likelihood function for either the probit or the logit model is accomplished by [iterative] nonlinear estimation methods. There are now several computer programs available for probit and logit analysis, and these programs are very inexpensive to run.
...
recession_to_spread() %>% 
  ...
  broom::augment(fit, type.predict = "response", newdata=tibble(spread = seq(from = -2.5, to = 1.4, by = .40)))
```
...
.content-box-greeen[
 AUC is the suggested indicator to evaluate your model!
]
